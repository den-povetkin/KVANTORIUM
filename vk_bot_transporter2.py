...
def first_point(user_id, is_first_point=True):
    """
    Обработка первой точки маршрута.
    Если is_first_point=True: пытается считать NFC метку, при неудаче предлагает ручной ввод.
    Если is_first_point=False: сразу предлагает ручной ввод (последующие точки).
    """
    global points_to_visit, waiting_for_coordinates
    
    if is_first_point:
        send_message(user_id, "Поднесите NFC метку для первой точки...")
        
        # Ожидание NFC метки в течение 10 секунд (20 попыток по 0.5 сек)
        nfc_detected = False
        for _ in range(20):
            ...
            if uid is not None:
                ...
                if text:
                    try:
                        ...
                        send_message(user_id, f"✅ Точка ({row}, {col}) добавлена через NFC!")
                        nfc_detected = True
                        break
        ...
        if nfc_detected:
            return  # Успешно добавлена через NFC
    
    # Если NFC не обнаружена (или is_first_point=False) - переходим к ручному вводу
    send_message(user_id, "📝 Введите координаты точки в формате: 'строка столбец' (например, '2 3'):")
...
def handle_message(user_id, text):
    ...
    if text_lower in ["старт", "начать", "start", "/start", "привет"]:
        ...
        threading.Thread(target=first_point, args=(user_id, True)).start()
    ...
    # Кнопки построения маршрута
        ...
        send_message(user_id, "📝 Добавление точки вручную...")
        threading.Thread(target=first_point, args=(user_id, False)).start()
    ...
    for event in longpoll.listen():
        if event.type == VkBotEventType.MESSAGE_NEW:
            if event.message.text:
                ...
                # Проверяем ожидание ввода координат
                if waiting_for_coordinates.get(user_id):
                    try:
                        ...
                        row = int(parts[0])
                        col = int(parts[1])
                        # Проверяем, что координаты в пределах карты (0-4 для 5x5)
                        if 0 <= row < 5 and 0 <= col < 5:
                            points_to_visit.append((row, col))
                            send_message(user_id, f"✅ Точка ({row}, {col}) добавлена вручную!")
                        else:
                            send_message(user_id, "❌ Координаты должны быть в диапазоне 0-4")
                    except (ValueError, IndexError):
                        send_message(user_id, "❌ Неверный формат. Введите две цифры через пробел, например: '2 3'")
